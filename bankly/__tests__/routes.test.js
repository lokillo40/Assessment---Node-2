...
beforeEach(async function () {
  async function _pwd(password) {
    if (typeof password !== "string") {
      console.error("Password value is not a string:", password);
...
describe("POST /auth/register", function () {
  ...
  // TESTS BUG #4
  test('should not return hashed password on registration', async function () {
    const response = await request(app)
      .post('/auth/register')
      .send({
        username: 'newuser',
...
describe("PATCH /users/[username]", function () {
  ...
  expect(response.status).toBe(200);
  expect(response.body.user.first_name).
...
// TESTS BUG #2
test('should delete user with given username if admin', async function () {
  const response = await request(app).delete('/users/testuser').set('Authorization', 'Bearer ADMIN_TOKEN');
  expect(response.status).toBe(200);
  expect(response.body).toEqual({ message: 'deleted' });
});
